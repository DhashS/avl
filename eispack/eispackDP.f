...
C
      ...
      IF (TI .LT. 0.0) S = -S
      IF (TR .LE. 0.0) YI = S
      IF (TR .LT. 0.0) YR = 0.5D0*(TI/YI)
      IF (TR .GT. 0.0) YI = 0.5D0*(TI/YR)
      RETURN
      END
...
C
     ...
     X        IERR,MAXJ,MAXK,GROUP
      ...
      REAL*8 U,V,UK,XU,X0,X1,E21,EPS2,EPS3,EPS4,NORM,ORDER,
     X       EPSLON,PYTHAG
C
...
C
   ...
   60    CONTINUE
C
         IF (E21 .LT. 0.0) NORM = 0.5D0 * NORM
C     .......... EPS2 IS THE CRITERION FOR GROUPING,
C                EPS3 REPLACES ZERO PIVOTS AND EQUAL
C                ROOTS ARE MODIFIED BY EPS3,
C                EPS4 IS TAKEN VERY SMALL TO AVOID OVERFLOW ..........
         IF (NORM .EQ. 0.0) NORM = 1.0
         EPS2 = 1.0D-3 * NORM * ABS(ORDER)
         EPS3 = EPSLON(NORM)
         UK = N
         UK = SQRT(UK)
         EPS4 = UK * EPS3
...
C
      ...
      REAL*8 U,V,LB,T1,T2,UB,XU,X0,X1,EPS1,TST1,TST2,EPSLON
      INTEGER IND(MM)
C
...
C
     ...
     X        M1,M2,M3,M4,NI,NM,NV,ITS,KJ1,M21,M31,IERR,IMULT
      ...
      REAL*8 F,G,Q,R,S,T,TST1,TST2,SCALE,PYTHAG
C
...
C
      ...
      REAL*8 AR(NM,N),AI(NM,N),WR(N),WI(N),ZR(NM,MM),
     X       ZI(NM,MM),RM1(N,N),RM2(N,N),RV1(N),RV2(N)
      REAL*8 X,Y,EPS3,NORM,NORMV,EPSLON,GROWTO,ILAMBD,PYTHAG,
     X       RLAMBD,UKROOT
      LOGICAL SELECT(N)
C
...
C
      ...
      REAL*8 SI,SR,TI,TR,XI,XR,YI,YR,ZZI,ZZR,TST1,TST2
C
...
C
      INTEGER I,J,M,LA,MM,MP,NM,IGH,KP1,LOW,MP1
      REAL*8 AR(NM,IGH),AI(NM,IGH),ORTR(IGH),ORTI(IGH),
     X       ZR(NM,M),ZI(NM,M)
      REAL*8 H,GI,GR
C
...
C     .......... FOR MP=IGH-1 STEP -1 UNTIL LOW+1 DO -- ..........
      DO 140 MM = KP1, LA
         MP = LOW + IGH - MM
         IF (AR(MP,MP-1) .EQ. 0.0 .AND. AI(MP,MP-1) .EQ. 0.0)
     X      GO TO 140
...
C     .......... SOLVE COMPLEX EQUATIONS ..........
  780       X = H(I,I+1)
            ...
            VI = (WR(I) - P) * 2.0 * Q
            IF (VR .NE. 0.0 .OR. VI .NE. 0.0) GO TO 784
...
C     .......... GUARD AGAINST UNDERFLOWED ELEMENT OF E2 ..........
            IF (E2(M+1) .EQ. 0.0) GO TO 125
  110    CONTINUE
C
  120    IF (M .LE. K) GO TO 130
         IF (M .NE. N) E2(M+1) = 0.0
  125    K = M
         TAG = TAG + 1
  130    P = W(L)
         IF (M .EQ. L) GO TO 215
...
C
      ...
      REAL*8 A(NM,N),WR(N),WI(N),Z(NM,MM),RM1(N,N),
     X       RV1(N),RV2(N)
      REAL*8 T,W,X,Y,EPS3,NORM,NORMV,EPSLON,GROWTO,ILAMBD,
...
C
         DO 780 I = 1, UK
            ...
            IF (ILAMBD .NE. 0.0) X = PYTHAG(RV1(I),RV2(I))
            IF (NORMV .GE. X) GO TO 760
            NORMV = X
            J = I
  760       NORM = NORM + X
...
C
      ...
      REAL*8 F,P,Q,R,S,EP,QP,ERR,TOT,EPS1,DELTA,EPSLON
      INTEGER IND(N)
      LOGICAL TYPE
C
...
C                TO REDUCE THE INCIDENCE OF UNDERFLOWS ..........
  190    IF (K .EQ. N) GO TO 210
         K1 = K + 1
         DO 200 J = K1, N
            IF (BD(J) .LE. (EPSLON(W(J)+W(J-1))) ** 2) BD(J) = 0.0
  200    CONTINUE
C
  210    F = BD(N) / DELTA
         QP = DELTA + F
...
C 
      ...
      REAL*8 A(NM,N),W(N),Z(NM,M),FWORK(1)
C
C     THIS SUBROUTINE CALLS THE RECOMMENDED SEQUENCE OF
C     SUBROUTINES FROM THE EIGENSYSTEM SUBROUTINE PACKAGE (EISPACK)
...
C
     ...
     X       RV1(N),RV2(N),RV3(N),RV4(N),RV6(N)
      REAL*8 U,V,UK,XU,X0,X1,EPS2,EPS3,EPS4,NORM,ORDER,EPSLON,
     X       PYTHAG
      INTEGER IND(M)
C
